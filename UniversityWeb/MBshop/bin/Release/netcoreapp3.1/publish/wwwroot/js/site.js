...
function PostMessages() {
    ...
    if (currentContent.length > 100) {
        var asd = document.getElementById('validation');
        asd.innerText = "To long sentance!";
    $("#validation").show().delay(2500).fadeOut();
        document.getElementById('input').value = "";
    }

    if (currentContent.length < 100 || currentContent.length > 0) {
            ...
            if (this.responseText != null) {
                ...
                if (newLenght != oldLenght) {
                    ...
                    for (var prop = newLenght - 1; newLenght > 0; prop--) {
                        ...
                        if (messageContent.length % 49){

                            countForIndex += 1;

                            var split = `<br/>`;
                            //inserting at index a br tag
                            String.prototype.splice = function (idx, rem, str) {
                                return this.slice(0, idx) + str + this.slice(idx + Math.abs(rem));
                            };

                            var indexSpliter = 49 * countForIndex;
                            console.log(indexSpliter);
                            var resulttes = messageContent.splice(indexSpliter, 0, split);
                            messageContent = resulttes;
                        }
